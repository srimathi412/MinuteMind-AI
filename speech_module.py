...
def split_audio(audio_path):
    ...
    Otherwise, optimizes the audio file (downsamples to mono, 16kHz, MP3) to reduce upload time.
    """
    ...
    if duration_mins > 10 or file_size > size_limit:
        ...
        for idx, chunk in enumerate(chunks):
            # Downsample chunk to mono, 16kHz for optimization and compress to mp3
            ...
            optimized_chunk.export(tmp_chunk_path, format="mp3", bitrate="64k")
            chunk_paths.append(tmp_chunk_path)
            
        return chunk_paths, True
    else:
        # For single files, we downsample to mono, 16kHz and compress as MP3 to optimize upload speed!
        optimized_audio = audio.set_frame_rate(16000).set_channels(1)
        
        with tempfile.NamedTemporaryFile(delete=False, suffix=".mp3") as tmp_file:
            tmp_path = tmp_file.name
            
        optimized_audio.export(tmp_path, format="mp3", bitrate="64k")
